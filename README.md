...
1. Download the installer for your platform from [Releases](https://github.com/nsuanningmeng/mc-forgelab/releases/latest):
   - Windows: `MC.ForgeLab.Setup.x.x.x.exe`
   - macOS (Intel): `MC.ForgeLab-x.x.x.dmg`
   - macOS (Apple Silicon): `MC.ForgeLab-x.x.x-arm64.dmg`
   - Linux: `MC.ForgeLab-x.x.x.AppImage`
2. Run the installer. On Windows you can choose the installation directory.
3. Launch **MC ForgeLab** — the embedded server starts automatically and opens in a browser window.

> On Linux, make the AppImage executable first: `chmod +x MC.ForgeLab-*.AppImage && ./MC.ForgeLab-*.AppImage`

**Docker (server / headless)**

```bash
cp docker-compose.example.yml docker-compose.yml
# Edit docker-compose.yml — set MC_FORGELAB_PROVIDER_SECRET and your AI provider key
docker compose up -d
...
1. 从 [Releases](https://github.com/nsuanningmeng/mc-forgelab/releases/latest) 下载对应平台的安装包：
   - Windows：`MC.ForgeLab.Setup.x.x.x.exe`
   - macOS（Intel）：`MC.ForgeLab-x.x.x.dmg`
   - macOS（Apple Silicon）：`MC.ForgeLab-x.x.x-arm64.dmg`
   - Linux：`MC.ForgeLab-x.x.x.AppImage`
2. 运行安装程序。Windows 安装时可自定义安装目录，安装后控制面板可卸载。
3. 启动 **MC ForgeLab**，内嵌服务器自动启动并在浏览器窗口中打开界面。

> Linux 用户需先赋予执行权限：`chmod +x MC.ForgeLab-*.AppImage && ./MC.ForgeLab-*.AppImage`

**Docker（服务器 / 无头部署）**

```bash
cp docker-compose.example.yml docker-compose.yml
# 编辑 docker-compose.yml，设置 MC_FORGELAB_PROVIDER_SECRET 和 AI 提供商的 API Key
docker compose up -d
